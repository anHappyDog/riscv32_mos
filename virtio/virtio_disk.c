...
void disk_init(void) {
	...
	if (GET(DISK_ADDRESS,DISK_MAGIC_VALUE) != MAGIC_VALUE  ||
		...
		debugf("%08x\n",GET(DISK_ADDRESS,DISK_DEVICEID));
		user_panic("failed to init the disk due to the check magic value ...\n");
	}
	...
	debugf("feathers in 31 - 0 bit are %08x\n",feathers);
	...
	debugf("after set feathers in 31 - 0 bit are %08x\n",feathers);
	...
	debugf("feathers in 63 -32 bit are %08x\n",feathers);
	GET(DISK_ADDRESS,DISK_DRIVERFEATHERS_SEL) = 1;
	GET(DISK_ADDRESS,DISK_DRIVERFEATHERS) = feathers;
	status |= FEATHERS_OK;
	GET(DISK_ADDRESS,DISK_STATUS) = status;
	if (!(status & FEATHERS_OK)) {
		user_panic("disk featehrs is not ok after set!\n");
	}
	if (GET(DISK_ADDRESS,DISK_QUEUEREADY)) {
		user_panic("disk shouldn't be ready before set!\n");
	}
	_u32 maxq = GET(DISK_ADDRESS,DISK_QUEUENUM_MAX);
	if (maxq == 0 || maxq < QUEUE_SIZE) {
		user_panic("max queuesize is not enough!\n");
	}	
	if (ecall_mem_alloc(ecall_getenvid(),(void*)DISK_DESC_ADDRESS,PTE_W | PTE_R | PTE_U | PTE_G) != 0) {
		user_panic("alloc page failed!\n");
	}
	disk.desc = (struct virtq_desc*)DISK_DESC_ADDRESS;
	if (ecall_mem_alloc(ecall_getenvid(),(void*)DISK_AVAIL_ADDRESS,PTE_W | PTE_R | PTE_U | PTE_G) != 0) {
		user_panic("alloc page failed!\n");
	}
	disk.avail = (struct virtq_avail*)DISK_AVAIL_ADDRESS;
	if (ecall_mem_alloc(ecall_getenvid(),(void*)DISK_USED_ADDRESS,PTE_W | PTE_R | PTE_U | PTE_G) != 0) {
		user_panic("alloc page failed!\n");
	}
	disk.used = (struct virtq_used*)DISK_USED_ADDRESS;
	disk.free = 0xff;
...
static void disk_rw_sector(_u64 sector,int write,void*buf) {
	...
	if (alloc_3desc(idx) == -1) {
		user_panic("disk_rw alloc 3 idx failed!\n");
	}
