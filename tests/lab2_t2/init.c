void page_strong_check(void) {
	...
	assert(PADDR(PTE_ADDR(boot_pgdir[0])) == page2addr(pp0));

	printk("va2pa(boot_pgdir, 0x0) is %x\n", va2pa(boot_pgdir, 0x0));
	printk("page2addr(pp1) is %x\n", page2addr(pp1));

	assert(va2pa(boot_pgdir, 0x0) == page2addr(pp1));
	assert(pp1->pp_ref == 1);
	assert(va2pa(boot_pgdir, BY2PG) == page2addr(pp2));
	assert(pp2->pp_ref == 1);
	assert(va2pa(boot_pgdir, 2 * BY2PG) == page2addr(pp3));
	assert(pp3->pp_ref == 1);
	...
	assert(va2pa(boot_pgdir, BY2PG) == page2addr(pp2));
	assert(va2pa(boot_pgdir, 2 * BY2PG) == page2addr(pp2));
	...
	assert(PADDR(PTE_ADDR(boot_pgdir[0])) == page2addr(pp0));
	assert(PADDR(PTE_ADDR(boot_pgdir[1])) == page2addr(pp1));
	boot_pgdir[0] = 0;
	boot_pgdir[1] = 0;
	assert(pp0->pp_ref == 2);
	assert(pp1->pp_ref == 1);
